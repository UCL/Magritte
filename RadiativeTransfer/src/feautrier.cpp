...
int feautrier (long ndep, double *S, double *dtau, double *u, Eigen::matrixXd *Lambda)
{
  ...
  double Bnd         = 1.0 + 2.0/dtau[ndep-1] + 2.0/dtau[ndep-1]/dtau[ndep-1];
  ...
  for (long n = 1; n < ndep-1; n++)
  {
    Lambda(n,n) = (1.0 + G[n+1]) / (F[n-1] + G[n+1] + F[n-1]*G[n+1]) / C[n];
  }
