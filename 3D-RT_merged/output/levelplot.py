import matplotlib.pyplot as plt
import numpy as np

xg,yg,zg, vx,vy,vz = np.loadtxt("input/ingrid.txt", unpack=True)
ngrid = np.shape(xg)[0]

data = np.loadtxt('output/level_populations.txt', skiprows=1)
nlev = np.shape(data)[1]

mean_intensity_data = np.loadtxt("output/mean_intensity.txt")
nrad = np.shape(mean_intensity_data)[0]

print( np.shape(mean_intensity_data) )

print("ngrid =", ngrid)
print("nlev =", nlev)

gridpoint = 0


fig = plt.figure()
ax1 = fig.add_subplot(211)

pop = np.zeros(ngrid)
mean_intensity = np.zeros(ngrid)

print(np.shape(data))

for level in range(nlev):
    for point in range(ngrid):
        pop[point] = data[point][level]

    ax1.plot(pop, label=level)

ax1.legend()
ax1.set_xlabel("x (grid point)")
ax1.set_ylabel("level populations")
ax1.set_yscale("log")

ax2 = fig.add_subplot(212)


for rad in range(nrad):
    for point in range(ngrid):

        mean_intensity[point] = mean_intensity_data[rad][point]

    ax2.plot(mean_intensity, label=rad)

ax2.legend()
ax2.set_xlabel("x (grid point)")
ax2.set_ylabel("mean intensity")
ax2.set_yscale("log")

fig.tight_layout()
fig.savefig("levels.pdf", bbox_inches='tight')

plt.show()
