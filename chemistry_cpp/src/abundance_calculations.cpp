...
/*-----------------------------------------------------------------------------------------------*/
/*                                                                                               */
/* abundance_calculations: Calculate abundances for each species                                 */
/*                                                                                               */
...
static void PrintFinalOutput(realtype *abundance, int ngrid, int nspec);
...
int calculate_abundances_(realtype *abundance, realtype *rate, realtype *density,
                          realtype *temperature, int *ngrid, int *nspec, int *nreac)
{
  ...
  long mxstep;                                               /* Maximum number of internal steps */
  
  int flag, status, nfails;                     /* CVODE return flag, status and failure counter */


  /* Define necessary external variables contained within the Fortran module CHEMISTRY_MODULE */
...
#pragma omp parallel for default(none) schedule(dynamic) \
  shared(ngrid, nspec, nreac, nelect, nthread, abundance, rate, density, temperature) \
  ...
  for (n=0; n<*ngrid; n++) {
    ...
    /* Store the number of threads being used (for grid point 0) */
...
/* PrintFinalOutput: Print the final abundances returned from CVODE for each particle            */
/*-----------------------------------------------------------------------------------------------*/
...
/* PrintFinalStatistics: Print the final statistics returned from the most recent call to CVODE  */
/*-----------------------------------------------------------------------------------------------*/
