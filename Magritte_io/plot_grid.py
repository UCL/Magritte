...
dx = [x_sorted[i+1]-x_sorted[i] for i in range(len(x)-1)]
dy = [y_sorted[i+1]-y_sorted[i] for i in range(len(y)-1)]
...
for gridp in new_IDs:
    n_x = int( (x[gridp] - x_min) / (x_max-x_min) * (NSTEPS_x-1) )
    n_y = int( (y[gridp] - y_min) / (y_max-y_min) * (NSTEPS_y-1) )
