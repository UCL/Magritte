...
{
  // Direction
  const double x[NRAYS];
  const double y[NRAYS];
  const double z[NRAYS];

  const long antipod[NRAYS];
  const long mirror_xz[NRAYS];

  HEALPIXVECTORS();
};


// HEALPIXVECTORS: constructor for HEALPIXVECTORS
// ----------------------------------------------
