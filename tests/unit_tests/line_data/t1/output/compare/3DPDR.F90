...
do pp=1,pdr_ptot
  temp = DUMMY_ABUNDANCE(4,pp)
  DUMMY_ABUNDANCE(4,pp) = DUMMY_ABUNDANCE(5,pp)
  DUMMY_ABUNDANCE(5,pp) = DUMMY_ABUNDANCE(6,pp)
  DUMMY_ABUNDANCE(6,pp) = temp
enddo
...
do pp=1,pdr_ptot
  temp = DUMMY_ABUNDANCE(5,pp)
  DUMMY_ABUNDANCE(5,pp) = DUMMY_ABUNDANCE(4,pp)
  DUMMY_ABUNDANCE(4,pp) = DUMMY_ABUNDANCE(6,pp)
  DUMMY_ABUNDANCE(6,pp) = temp
enddo
...
!
!
! Write LTE populations
   !
   ! write(tag,'(I0.0)') pp
   ! write(6,*) tag
   ! open(unit=999,file="frederik_output/R_C_" // trim(tag) // "_3D-PDR.txt",status='replace')
   ! do iindex=1,CI_nlev
   !   do jindex=1,CI_nlev
   !     write(999, '(E$)') PDR(P)%CII_POP
   !   enddo
   !   write(999,*)
   ! enddo
   ! close(999)
...
IF (pp.EQ.1) THEN
  ...
  open(unit=999,file="output/info_3D-PDR.txt",status='replace')
  write(999, '(E$)') gastemperature(pp)
  write(999,*)
  write(999, '(E$)') pdr(p)%abundance(NH2)*pdr(p)%rho
  close(999)

  open(unit=999,file="output/Einstein_A_CO_3D-PDR.txt",status='replace')
  do iindex=1,C12O_NLEV
    do jindex=1,C12O_NLEV
      write(999, '(E$)') C12O_A_COEFFS(iindex,jindex)
    enddo
    write(999,*)
  enddo
  close(999)

  open(unit=999,file="output/Einstein_B_CO_3D-PDR.txt",status='replace')
  do iindex=1,C12O_NLEV
    do jindex=1,C12O_NLEV
      write(999, '(E$)') C12O_B_COEFFS(iindex,jindex)
    enddo
    write(999,*)
  enddo
  close(999)

  open(unit=999,file="output/Einstein_C_CO_3D-PDR.txt",status='replace')
  do iindex=1,C12O_NLEV
    do jindex=1,C12O_NLEV
      write(999, '(E$)') C12O_C_COEFFS(iindex,jindex)
    enddo
    write(999,*)
  enddo
  close(999)
