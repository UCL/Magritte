...
C:
      ...
      PARAMETER(mdep=200)
      double precision a1(mdep),c1(mdep),dtauq(mdep),s(mdep),sol(mdep)
      double precision p(mdep), IBC
C: Open ray file
      ntrani=0
      open(10,file='feautrier1.txt',status='old')
...
C read in source function and optical depth at each cell k
      do 10 i=1,1000,1
        read(10,*,end=20) s(i),dtauq(i),sol(i)
        ntrani = ntrani+1
 10   continue
 20   close(10)
...
C write output to the output file
      open(11,file='feautrier1.txt',status='unknown')
      do 130 i=1,ntrani
