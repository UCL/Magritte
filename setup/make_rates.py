        ...
        for i in range(nReactions):
            if reactants[i].count(species) > 0:
                if is_H2_formation(reactants[i], products[i]):
                    lossString += '-2*cell[gridp].rate['+str(i)+']*n_H'
                    continue
                lossString += '-'+multiple(reactants[i].count(species))+'cell[gridp].rate['+str(i)+']'
            ...
            if products[i].count(species) > 0:
                if is_H2_formation(reactants[i], products[i]):
                    formString += '+cell[gridp].rate['+str(i)+']*Ith(y,'+str(speciesList.index('H'))+')*n_H'
                    continue
                formString += '+'+multiple(products[i].count(species))+'cell[gridp].rate['+str(i)+']'
                for reactant in speciesList:
                    for j in range(reactants[i].count(reactant)):
        ...
        for m in range(nSpecies):
            ...
            for i in range(nReactions):
                if reactants[i].count(species1) > 0 and reactants[i].count(species2) > 0:
                    if is_H2_formation(reactants[i], products[i]):
                        matrixString += '-2*cell[gridp].rate['+str(i)+']*n_H'
                        continue
                    matrixString += '-'+multiple(reactants[i].count(species1))+multiple(reactants[i].count(species1))+'cell[gridp].rate['+str(i)+']'
                    for reactant in speciesList:
                    ...
                    if reactants[i].count('XRSEC') == 1:
                        if reactants[i].count('H') == 1:
                            matrixString = matrixString[:-len('-cell[gridp].rate['+str(i)+']')]
                            additionalString += '  IJth(J,'+str(formatCode % indexH2)+','+str(formatCode % n)+') -= cell[gridp].rate['+str(i)+']*Ith(y,'+str(indexH) +')*zeta_H*(-1.89/(Ith(y,'+str(indexH)+')+1.89*Ith(y,'+str(indexH2)+')));\n'
                            additionalString += '  IJth(J,'+str(formatCode % indexH) +','+str(formatCode % n)+') -= cell[gridp].rate['+str(i)+']*Ith(y,'+str(indexH2)+')*zeta_H*(+1.89/(Ith(y,'+str(indexH)+')+1.89*Ith(y,'+str(indexH2)+')));\n'
                        elif reactants[i].count('H2') == 1:
                            matrixString = matrixString[:-len('-cell[gridp].rate['+str(i)+']')]
                            additionalString += '  IJth(J,'+str(formatCode % indexH2)+','+str(formatCode % n)+') -= cell[gridp].rate['+str(i)+']*Ith(y,'+str(indexH) +')*zeta_H2*(+0.53/(Ith(y,'+str(indexH2)+')+0.53*Ith(y,'+str(indexH)+')));\n'
                            additionalString += '  IJth(J,'+str(formatCode % indexH) +','+str(formatCode % n)+') -= cell[gridp].rate['+str(i)+']*Ith(y,'+str(indexH2)+')*zeta_H2*(-0.53/(Ith(y,'+str(indexH2)+')+0.53*Ith(y,'+str(indexH)+')));\n'
                        elif reactants[i].count('He') == 1:
                            matrixString += '*zeta_He'
                        else:
                            matrixString += '*zeta_H'
                            additionalString += '  IJth(J,'+str(formatCode % indexH2)+','+str(formatCode % n)+') -= cell[gridp].rate['+str(i)+']*Ith(y,'+str(n)+')*zeta_H*(-1.89/(Ith(y,'+str(indexH)+')+1.89*Ith(y,'+str(indexH2)+')));\n'
                            additionalString += '  IJth(J,'+str(formatCode % indexH) +','+str(formatCode % n)+') -= cell[gridp].rate['+str(i)+']*Ith(y,'+str(n)+')*zeta_H*(-1.00/(Ith(y,'+str(indexH)+')+1.89*Ith(y,'+str(indexH2)+')));\n'

                    # Photoreactions due to X-ray induced secondary photons (Lyman-alpha from excited H)
                    if reactants[i].count('XRLYA') == 1:
                        matrixString += '*Ith(y,'+str(indexH)+')*zeta_H'
#                        additionalString += '  IJth(J,'+str(formatCode % indexH2)+','+str(formatCode % n)+') -= cell[gridp].rate['+str(i)+']*Ith(y,'+str(n)+')*zeta_H*(-1.89*Ith(y,'+str(indexH)+ ')/(Ith(y,'+str(indexH)+')+1.89*Ith(y,'+str(indexH2)+')));\n'
#                        additionalString += '  IJth(J,'+str(formatCode % indexH) +','+str(formatCode % n)+') -= cell[gridp].rate['+str(i)+']*Ith(y,'+str(n)+')*zeta_H*(+1.89*Ith(y,'+str(indexH2)+')/(Ith(y,'+str(indexH)+')+1.89*Ith(y,'+str(indexH2)+')));\n'

                    # Photoreactions due to X-ray induced secondary photons (Lyman-Werner from excited H2)
                    if reactants[i].count('XRPHOT') == 1:
                        matrixString += '*Ith(y,'+str(indexH2)+')*zeta_H2'
#                        additionalString += '  IJth(J,'+str(formatCode % indexH2)+','+str(formatCode % n)+') -= cell[gridp].rate['+str(i)+']*Ith(y,'+str(n)+')*zeta_H2*(+0.53*Ith(y,'+str(indexH)+ ')/(Ith(y,'+str(indexH2)+')+0.53*Ith(y,'+str(indexH)+')));\n'
#                        additionalString += '  IJth(J,'+str(formatCode % indexH) +','+str(formatCode % n)+') -= cell[gridp].rate['+str(i)+']*Ith(y,'+str(n)+')*zeta_H2*(-0.53*Ith(y,'+str(indexH2)+')/(Ith(y,'+str(indexH2)+')+0.53*Ith(y,'+str(indexH)+')));\n'

            # Formation terms for species1
            for i in range(nReactions):
                if products[i].count(species1) > 0 and reactants[i].count(species2) > 0:
                    if is_H2_formation(reactants[i], products[i]):
                        matrixString += '+cell[gridp].rate['+str(i)+']*n_H'
                        continue
                    matrixString += '+'+multiple(products[i].count(species1))+'cell[gridp].rate['+str(i)+']'
                    for reactant in speciesList:
                        if reactant == species2:
                            for j in range(reactants[i].count(reactant)-1):
                                matrixString += '*Ith(y,'+str(speciesList.index(reactant))+')*n_H'
                    ...
                    if reactants[i].count('XRSEC') == 1:
                        if reactants[i].count('H') == 1:
                            ...
                            matrixString += '*zeta_H'
                            additionalString += '  IJth(J,'+str(formatCode % indexH2)+','+str(formatCode % n)+') += cell[gridp].rate['+str(i)+']*Ith(y,'+str(m)+')*zeta_H*(-1.89/(Ith(y,'+str(indexH)+')+1.89*Ith(y,'+str(indexH2)+')));\n'
                            additionalString += '  IJth(J,'+str(formatCode % indexH) +','+str(formatCode % n)+') += cell[gridp].rate['+str(i)+']*Ith(y,'+str(m)+')*zeta_H*(-1.00/(Ith(y,'+str(indexH)+')+1.89*Ith(y,'+str(indexH2)+')));\n'

                    # Photoreactions due to X-ray induced secondary photons (Lyman-alpha from excited H)
                    if reactants[i].count('XRLYA') == 1:
                        matrixString += '*Ith(y,'+str(indexH)+')*zeta_H'
#                        additionalString += '  IJth(J,'+str(formatCode % indexH2)+','+str(formatCode % n)+') += cell[gridp].rate['+str(i)+']*Ith(y,'+str(m)+')*zeta_H*(-1.89*Ith(y,'+str(indexH)+ ')/(Ith(y'+str(indexH)+')+1.89*Ith(y'+str(indexH2)+')));\n'
#                        additionalString += '  IJth(J,'+str(formatCode % indexH) +','+str(formatCode % n)+') += cell[gridp].rate['+str(i)+']*Ith(y,'+str(m)+')*zeta_H*(+1.89*Ith(y,'+str(indexH2)+')/(Ith(y'+str(indexH)+')+1.89*Ith(y'+str(indexH2)+')));\n'

                    # Photoreactions due to X-ray induced secondary photons (Lyman-Werner from excited H2)
                    if reactants[i].count('XRPHOT') == 1:
                        matrixString += '*Ith(y,'+str(indexH2)+')*zeta_H2'
#                        additionalString += '  IJth(J,'+str(formatCode % indexH2)+','+str(formatCode % n)+') += cell[gridp].rate['+str(i)+']*Ith(y,'+str(m)+')*zeta_H2*(+0.53*Ith(y,'+str(indexH)+ ')/(Ith(y'+str(indexH2)+')+0.53*Ith(y'+str(indexH)+')));\n'
#                        additionalString += '  IJth(J,'+str(formatCode % indexH) +','+str(formatCode % n)+') += cell[gridp].rate['+str(i)+']*Ith(y,'+str(m)+')*zeta_H2*(-0.53*Ith(y,'+str(indexH2)+')/(Ith(y'+str(indexH2)+')+0.53*Ith(y'+str(indexH)+')));\n'
